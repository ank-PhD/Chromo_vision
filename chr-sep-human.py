...
if __name__ == "__main__":
    ...
    # lg1, lg2, _, _ = gabor(bw, 1/64, 8, field=160)
    #
    # plt.subplot(2,2,1)
    ...
    # plt.imshow(lg1, cmap='gray', interpolation='nearest')
    # plt.colorbar()
    #
    # plt.subplot(2,2,3)
    # plt.title('Gabor - line detector, positive compound only')
    # # plt.imshow(lg2, cmap='gray', interpolation='nearest')
    # plt.colorbar()
    #
    # plt.subplot(2,2,4)
    ...
    plt.subplot(2,2,3)
    plt.title('Segmentation - total time %s'%"{0:.2f}".format(time()-start))
    plt.imshow(label(d_c, background=0)*(d_c-1), cmap='spectral', interpolation='nearest')
    plt.colorbar()
